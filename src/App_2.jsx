...
function Stats({ log, meta, decks, onBack, onRestore, onSignOut, userEmail }) {
  ...
  const exportData=(list)=>{ try{
    const dk = (list && list.length) ? list : decks;
    const ids = new Set(dk.map((d)=>d.id));
    const lg = log.filter((e)=>ids.has(e.deck));
    const blob=new Blob([JSON.stringify({app:"repaso",version:1,exported:new Date().toISOString(),decks:dk,meta,log:lg})],{type:"application/json"});
  ...
  const openExp=()=>{ const s={}; decks.forEach((d)=>{ s[d.id]=true; }); setExpSel(s); setPickExp(true); };
  const expChosen=decks.filter((d)=>expSel[d.id]);
  ...
  return <div className="rise">
    ...
    <div style={{background:C.surface,borderRadius:18,padding:16,marginTop:16,boxShadow:"0 2px 10px rgba(34,50,44,.05)"}}>
      ...
      <div className="flex gap-2" style={{flexWrap:"wrap"}}>
        <button onClick={()=>exportData()} className="flex items-center gap-2" style={{...primaryBtn,flex:1,minWidth:150}}><Download size={16} /> Exportar todo</button>
      ...
      {decks.length>1 && (pickExp
        ? <div className="rise" style={{marginTop:10,padding:12,borderRadius:12,background:C.bg}}>
            <div className="flex items-center justify-between" style={{marginBottom:8}}>
              <span style={{fontSize:13.5,fontWeight:800}}>Elige mazos a exportar</span>
              <button onClick={()=>setPickExp(false)} style={iconBtn}><X size={15} /></button>
            </div>
            <div className="grid gap-1">
              {decks.map((d)=><label key={d.id} className="flex items-center gap-2" style={{fontSize:13.5,padding:"5px 4px",borderRadius:8,cursor:"pointer"}}>
                <input type="checkbox" checked={!!expSel[d.id]} onChange={()=>setExpSel((se)=>({...se,[d.id]:!se[d.id]}))} style={{accentColor:C.teal,width:16,height:16}} />
                <span style={{fontSize:16}}>{d.emoji}</span>
                <span style={{flex:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{d.name}</span>
                <span style={{color:C.muted}}>{d.cards.length}</span>
              </label>)}
            </div>
            <button disabled={!expChosen.length} onClick={()=>{exportData(expChosen);setPickExp(false);}} className="flex items-center justify-center gap-2" style={{...primaryBtn,width:"100%",marginTop:10,opacity:expChosen.length?1:0.5}}>
              <Download size={15} /> Exportar {expChosen.length} mazo{expChosen.length===1?"":"s"}
            </button>
          </div>
        : <button onClick={openExp} style={{...linkBtn,color:C.tealDeep,marginTop:8,fontSize:13}}>Elegir mazos a exportar…</button>)}
      <p style={{color:C.rose,fontSize:11.5,margin:"10px 0 0"}}>Importar reemplaza todo lo que tienes ahora.</p>
