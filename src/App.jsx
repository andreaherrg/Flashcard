...
function Study({ session, decks, updateDeck, onReview, onBack, onHome }) {
  ...
  const [queue,setQueue]=useState(()=>{
    ...
    for(const id of session.deckIds){ const d=byId[id]; if(!d) continue;
      ...
      if(session.includeNew!==false) newCardsOf(d).slice(0,newAllowed(d)).forEach((c)=>refs.push({deckId:id,cardId:c.id}));
    }
  ...
  return <>
    ...
    <div style={{perspective:1600,marginBottom:22}} onClick={()=>setFlipped((f)=>!f)}>
      <div className={"card3d"+(flipped?" flip":"")} style={{position:"relative",minHeight:300}}>
        ...
        <div className="face back" style={{...faceStyle(C.surface),background:`linear-gradient(${color}12, ${color}12), ${C.surface}`,position:"absolute",inset:0,borderColor:color+"40",overflow:"auto"}}>
